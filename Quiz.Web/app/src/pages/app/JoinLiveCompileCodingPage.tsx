        ...
        <div className="w-full space-y-5">
            <div className="rounded-3xl border border-slate-900/10 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-slate-900/55">
                <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                    <div className="space-y-3">
                        <div>
                            <div className="text-xl font-extrabold tracking-tight text-slate-900 dark:text-white">
                                {state.title || "Live Coding (Compile)"}
                            </div>
                            <div className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                                Sesiune <span className="font-mono font-bold text-slate-700 dark:text-slate-200">{sessionCode}</span> • {state.tasks.length} sarcini
                            </div>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-2">
                            ...
                            {selectedTask && (
                                <span className="rounded-full bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 ring-1 ring-slate-200 dark:bg-slate-950/40 dark:text-slate-200 dark:ring-white/10">
                                    Sarcina activă: {selectedTask.title}
                                </span>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                        <span className="rounded-2xl bg-emerald-50 px-4 py-2 text-sm font-semibold text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300">
                            Limbaj: {languageLabel(selectedLanguage)}
                        </span>
            ...
            <div className="rounded-3xl border border-slate-900/10 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-slate-900/55">
                <div className="mb-4 flex flex-col gap-1">
                    <div className="text-sm font-semibold text-slate-900 dark:text-slate-100">Sarcini disponibile</div>
                    <div className="text-xs text-slate-500 dark:text-slate-400">
                        Selectează rapid sarcina pe care vrei să lucrezi. Fiecare card îți arată și cel mai bun scor obținut până acum.
                    </div>
                </div>
                    ...
                    {state.tasks.map((task, index) => (
                        <button
                            ...
                            className={cn(
                                ...
                                task.id === selectedTask?.id
                                    ? "border-indigo-500 bg-indigo-50 text-indigo-700 shadow-sm shadow-indigo-500/10 dark:border-indigo-400 dark:bg-indigo-500/10 dark:text-indigo-300"
                        ...
                        >
                            <div className="flex flex-wrap items-start justify-between gap-3">
                                ...
                                {task.id === selectedTask?.id && (
                                    <span className="rounded-full bg-white/80 px-2 py-1 text-[11px] font-bold uppercase tracking-wide dark:bg-slate-950/50">
                                        Activă
                                    </span>
                                )}
                            </div>
                            <div className="mt-3 flex items-center justify-between text-xs font-medium">
                                <span>{task.points} puncte</span>
            ...
            <div className="rounded-3xl border border-slate-900/10 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-slate-900/55">
                <div className="mb-4 flex flex-col gap-1">
                    <div className="text-sm font-semibold text-slate-900 dark:text-slate-100">Limbaj de programare</div>
                    <div className="text-xs text-slate-500 dark:text-slate-400">
                        Poți schimba limbajul oricând. Draft-ul se păstrează separat pentru fiecare combinație sarcină + limbaj.
                    </div>
                </div>
            ...
            {selectedTask && (
                <div className="rounded-3xl border border-slate-900/10 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-slate-900/55">
                    <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                        <div>
                            <div className="text-2xl font-bold text-slate-900 dark:text-white">{selectedTask.title}</div>
                            <div className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                                {selectedTask.points} puncte • Cel mai bun scor al tău: {state.playerTaskScores[selectedTask.id] ?? 0}p
                            </div>
                        </div>
                    ...
                    <div className="mt-4 grid grid-cols-1 gap-4 xl:grid-cols-3">
                        ...
                        <div className="rounded-2xl bg-slate-50 p-5 dark:bg-slate-950/40">
                            ...
                            <div className="whitespace-pre-wrap text-sm leading-7 text-slate-700 dark:text-slate-200">
                                {selectedTask.outputDescription}
                            </div>
                        </div>
                        ...
                        <div className="rounded-2xl bg-slate-50 p-5 dark:bg-slate-950/40">
                            ...
                            <div className="space-y-3">
                                <div>
                                    ...
                                    <pre className="overflow-auto rounded-xl bg-slate-950 px-3 py-3 text-xs text-slate-100">{selectedTask.exampleInput || "(gol)"}</pre>
                                </div>
                                <div>
                                    <div className="mb-1 text-xs font-semibold text-slate-500 dark:text-slate-400">Output</div>
                                    <pre className="overflow-auto rounded-xl bg-slate-950 px-3 py-3 text-xs text-slate-100">{selectedTask.exampleOutput || "(gol)"}</pre>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div className="rounded-3xl border border-slate-900/10 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-slate-900/55">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div>
                        <div className="text-base font-semibold text-slate-900 dark:text-slate-100">Editor soluție</div>
                        <div className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                    ...
                    <button
                        ...
                        className="rounded-2xl bg-indigo-600 px-6 py-3 text-sm font-bold text-white transition hover:bg-indigo-700 disabled:opacity-50 dark:bg-indigo-500 dark:hover:bg-indigo-400"
                    >
                ...
                <textarea
                    ...
                    spellCheck={false}
                    className="mt-4 h-[460px] w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4 font-mono text-sm text-slate-900 outline-none focus:ring-2 focus:ring-indigo-500/40 dark:border-white/10 dark:bg-slate-950 dark:text-slate-100"
            ...
            <div className="rounded-3xl border border-slate-900/10 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-slate-900/55">
                <div className="grid grid-cols-1 gap-5 xl:grid-cols-[minmax(0,1.6fr)_minmax(280px,0.9fr)]">
                    <div>
                        <div className="mb-3 text-sm font-semibold text-slate-700 dark:text-slate-300">Clasament live</div>
                        <LeaderboardPanel entries={state.leaderboard} currentDisplayName={displayName} />
                    </div>

                    <div className="rounded-2xl bg-slate-50 p-4 text-sm leading-6 text-slate-600 dark:bg-slate-950/40 dark:text-slate-300">
